...
//  개요(Overview)
//  ----------------------------------------------------------------------------
//  ChannelController는 단일 DDR4 메모리 채널에 대해
//  채널 레벨 중재(arbitration)와 타이밍 조율을 수행한다.
//
//  이 모듈은 다음 구성 요소들을 연결하는 통합 계층 역할을 한다:
//
//      • MC Frontend (요청 주입)
//      • 다수의 RankController (랭크 레벨 스케줄링 및 Row/Bank FSM)
//      • Memory Buffer (데이터 이동 조율)
//      • PHY Controller (명령/데이터 버스 실행)
//
//  RankController가 Row/Bank 레벨 타이밍과
//  Open-page 동작을 관리하는 반면,
//  ChannelController는 모든 랭크에 걸쳐
//  채널 레벨의 구조적/타이밍 제약을 강제한다.
//
//  ----------------------------------------------------------------------------
//  아키텍처적 역할(Architectural Role)
//  ----------------------------------------------------------------------------
//
//  1) 프론트엔드 역다중화(Frontend Demultiplexing)
//     - 들어오는 메모리 요청을
//       랭크 주소에 따라 적절한 RankController로 전달.
//     - 각 랭크의 ready 신호를 수집하여
//       프론트엔드로 전달.
//
//  2) 채널 레벨 명령 중재 (CMD Bus)
//     - 공유된 명령/주소 버스에 대한
//       독점적 접근 권한을 부여.
//     - 랭크 간 전환 제약(tRTR)을 강제.
//     - 한 사이클에 하나의 랭크만
//       명령을 발행하도록 보장.
//
//  3) 채널 레벨 데이터 스케줄링 (DQ Bus)
//     - CAS 간 간격 타이밍(tCCD_S / tCCD_L) 강제.
//     - Read/Write 전환 제약(tRTW, tWTR_S/L) 강제.
//     - 모든 랭크 간 데이터 버스 사용을 조율.
//
//  4) PHY 및 메모리 버퍼 연동
//     - PHY로부터 전달되는 Auto-Precharge 완료 ACK를
//       올바른 RankController로 전달.
//     - 랭크별 명령 ACK를 수집하여
//       Memory Buffer와 PHY Controller로 전달.
//     - 발행된 명령 추적을 유지하여
//       데이터 이동 정렬을 보장.
//
//
//  ----------------------------------------------------------------------------
//  타이밍 책임 경계(Timing Responsibility Boundary)
//  ----------------------------------------------------------------------------
//  ChannelController가 강제하는 항목:
//
//      • 채널 레벨 구조적 hazard
//      • CMD 버스 전환 제약 (tRTR)
//      • DQ 버스 구조적/전환 제약
...
//
//  [1] CMD 도메인
//      - 모든 랭크를 통틀어 사이클당 하나의 명령만 허용.
//      - 큐 깊이 및 FSM 준비 상태 기반 중재.
//      - 서로 다른 랭크 간 tRTR 강제.
//
//  [2] DQ 도메인
//      - 데이터 전송은 다음 제약을 따름:
//          • CAS-to-CAS 간격 (tCCD_S/L)
//          • Read/Write 방향 전환 제약
//      - CMD 중재와는 독립적이지만
//        ACK를 통해 상호 연동됨.
//
...
//        채널 레벨 타이밍과 랭크 레벨 타이밍을
//        엄격히 분리함.
//
//  • 구조적 hazard 인지
//        공유 자원(CMD 버스, DQ 버스)을
//        명시적으로 중재함.
//
//  • 결정적 타이밍 강제
//        모든 전환 제약을
...
//
//  • 확장 가능한 다중 랭크 구조
//        각 채널은 여러 RankController를 병렬로 포함하고,
//        ChannelController가 공유 자원 사용을 조율함.
//
//
//  ----------------------------------------------------------------------------
...
//  ----------------------------------------------------------------------------
//
//  - 채널당 하나의 ChannelController가 존재.
//  - 현재 구현은 NUMRANK = 4를 가정
//    (벡터화된 로직).
//  - 중재 로직은 다음 모듈들로 분리되어 있음:
//        • CMDGrantScheduler   (주 랭크 CMD 중재)
//        • CMDTurnaroundGrant  (tRTR 타이밍 계산)
//        • DQRdWrCCDGrant      (tCCD_S / tCCD_L 계산)
//        • DQTurnaroundGrant   (tRTW, tWTR_S / tWTR_L 계산)
//  - ChannelController는 요청 재정렬을 수행하지 않으며,
//    채널 레벨 충돌만 해결함.
//
//  작성자  : 조성원
//  작성일  : 2026.02
