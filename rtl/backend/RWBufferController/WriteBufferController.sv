...
//          - 캐시 측 프런트엔드로부터 전달되는 쓰기 데이터를 버퍼링.
//          - RankFSM에서 발행된(write) 명령을 추적.
//          - 버스트 단위의 쓰기 데이터를 순서대로(in-order) PHY로 스트리밍.
//          - 프런트엔드에 대한 쓰기 응답(ACK) 생성.
//
//      범위 및 참고 사항(Scope & Notes):
//          - 데이터 레벨 버퍼링만 담당함
//            (DRAM 타이밍 로직은 포함하지 않으며,
//             타이밍 계산은 PHY Controller에서 수행).
//          - 하나의 outstanding write 엔트리는
//            하나의 write burst에 대응됨.
//          - PHY 측 백프레셔(backpressure)는
//            FIFO ready 신호를 통해 처리됨.
//
//      작성자  : Seongwon Jo
//      작성일  : 2026.02
//------------------------------------------------------------------------------
