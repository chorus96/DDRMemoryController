...
//
//      역할(Role):
//          DDR PHY와 메모리 컨트롤러 프런트엔드 사이에 위치한
//          읽기(Read) 데이터 버퍼.
//
//      주요 책임(Responsibilities):
//          - 들어오는 Read 명령에 대해 Read 버퍼 엔트리를 할당.
//          - PHY로부터 수신되는 DQ 데이터를
//            미완료(outstanding)된 Read 요청과 매칭.
//          - 버스트 단위의 데이터를 버퍼링하고,
//            캐시로 순서대로(in-order) 전달.
//          - 요청 윈도우(request window)를 사용하여
//            랭크별 outstanding read 요청을 추적.
//
//      범위 및 참고 사항(Scope & Notes):
//          - 데이터 레벨의 순서 보장만 담당함 (버스트 단위).
//          - DRAM 타이밍 로직은 포함하지 않음
//            (RankExecutionUnit / PHY에서 처리).
//          - PHY Read 응답은 태그(tag) 기반으로 매칭함.
//
//      작성자  : Seongwon Jo
//      작성일  : 2026.02
//------------------------------------------------------------------------------
