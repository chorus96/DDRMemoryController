...
//      |  MemoryControllerFrontend |
//      |  - 단순화된 AXI 프로토콜   |
//      |  - 요청 조립(Request assembly)
//      +---------------------------+
//              |
//              v
//      +---------------------------+
//      |   MemoryController (본 모듈) |
//      |   - 채널 선택(Channel selection)
//      |   - 응답 중재(Response arbitration)
//      +---------------------------+
//           |               |
//           v               v
//      +-----------+   +-----------+
...
//          1) 메모리 요청을 올바른 채널 컨트롤러로 디스패치.
//          2) 기아(starvation)를 방지하기 위해
//             채널 레벨의 응답 중재 수행
//             (즉, 캐시 측으로 전달되는 응답 중재).
//          3) 프론트엔드 AXI 유사 인터페이스
//             (단순화된 AXI 버스 프로토콜)와
//             DDR4 백엔드를 연결.
//          4) 채널 간 부하가 비대칭일 때도
//             공정성(fairness)을 유지.
...
//          - 프론트엔드에서 한 번에 하나의 요청만 발행됨.
//          - 여러 채널의 응답은 동시에 반환될 수 있음.
//          - 응답 중재는 큐 깊이를 고려하며
//            기아 방지를 위한 Aging 기법을 사용.
//
//      작성자  : 조성원
//      작성일  : 2026.02
//////////////////////////////////////////////////////////////////////////////////////////`ifndef VERILATOR
